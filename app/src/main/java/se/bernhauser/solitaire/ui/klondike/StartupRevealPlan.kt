...
import se.bernhauser.solitaire.ui.board.ColumnRevealPlan
...
internal data class StartupRevealPlan(
  ...
  override fun coveredIndex(column: Int, row: Int): Int? = tableauCoveredIndex(column, row)

  override fun faceUpIndex(column: Int, row: Int): Int? = tableauFaceUpIndex(column, row)
...
internal fun buildStartupRevealPlan(state: KlondikeState): StartupRevealPlan {
  var nextCovered = 0
